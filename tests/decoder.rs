...
fn compare_definitions(original: &Value, reemitted: &Value) -> Result<(), String> {
    ...
    // For pa-block fallbacks (anything not InitializeVariable / SetVariable
    // / Increment / Decrement / AppendTo*), the action JSON is copied
    // verbatim into pa/<Name>.json and emitted unchanged. We assert these
    // bodies match key-for-key.
