...
Inside the JSON file you write Power Automate expression strings directly: variables as `variables('name')`, Compose outputs as `outputs('Compose_name')`, and trigger data as `triggerBody()`. This is the one place where you write PA expression syntax rather than pax syntax, single quotes and all. PA has two forms and the difference is load-bearing. A value that is nothing but an expression is written bare, `"@variables('total')"`, and keeps its type, which is what an integer or boolean parameter needs. A value that mixes text with expressions interpolates them, one pair of braces each: `"<p>Hello @{variables('name')}</p>"`, always producing a string. The convention is for the file to match PA's "Code View" / "Peek code" output exactly, which makes round-trip from a real PA flow byte-identical.
